...
public class MySampleVisitor extends SampleBaseVisitor<String> {
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (expressions.get(i)) {
                ...
                default: {
                    ...
                    if (existingVars.containsKey(currentVar)) {
                        result.add(existingVars.get(currentVar) + " var" + (counter) + " = " + expression);
                        counter++;
                    }
                }
            }
            ...
            switch (expressions.get(i)) {
                case "readint()": {
                    if (!existingVars.containsKey(currentVar)) {
                        ...
                        existingVars.put(currentVar, "int");
                    }
                    result.add("scanf(\"%i\", &" + currentVar + ");");
                    break;
                }
                case "readdouble()": {
                    if (!existingVars.containsKey(currentVar)) {
                        result.add(String.format("double %s;", currentVar));
                        existingVars.put(currentVar, "double");
                    }
                ...
                case "readbool()": {
                    if (!existingVars.containsKey(currentVar)) {
                        ...
                        existingVars.put(currentVar, "bool");
                    }
                    result.add("scanf(\"%b\", &" + currentVar + ");");
                    break;
                }
                default: {
                    List<String> typeAndExpression = Arrays.asList(expressions.get(i).split(":"));
                    ...
                    if (!existingVars.containsKey(currentVar)) {
                        ...
                        existingVars.put(currentVar, type);
                    }
                    result.add(currentVar + " = " + "var" + (oldcounter++) + ";");
                }
            }
        }
        return String.join("\n", result);
    ...
    private Map<String, String> existingVars = new HashMap<>();
}
